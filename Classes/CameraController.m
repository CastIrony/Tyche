...
-(BOOL)isAutomatic
{
    if(!_isAutomatic) { _isAutomatic = [[NSUserDefaults standardUserDefaults] objectForKey:@"automatic_camera"]; }
    if(!_isAutomatic) { _isAutomatic = [NSNumber numberWithBool:YES]; }
...
-(void)updateCamera
{
    if(self.menuVisible)
    {
        if(self.renderer.animated)
        {
            ...
            self.position    = [AnimatedVector3D withStartValue:self.position.value    endValue:Vector3DMake(0, 0.0, -9) forTime:1]; self.position.curve    = AnimationEaseInOut;
        ...
        else 
        {
            ...
            self.position    = [AnimatedVector3D withValue:Vector3DMake(0, 0.0, -9)]; 
